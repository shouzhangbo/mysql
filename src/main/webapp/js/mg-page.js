		...
		success: function(data){
			...
			if('0000'==data.respCode){
				...
				for(var i=0;i<data.list.length;i++){
					str = str + '<tr>';
					str = str + '<td><input type="checkbox" /><td>'+data.list[i].pageId+'</td><td>'+data.list[i].pageName+'</td><td>'+data.list[i].pageUrl+'</td><td>'+data.list[i].indexs+'</td>';
					str = str + '<td><img src="../../images/youxi-02.png" /></td><td>'+data.list[i].statusName+'</td><td>2015-01-11 10:00:11</td>';
	...
	$.ajax({
		...
		success: function(data){
			...
			if('0000'==data.respCode){
				var str = '';
				for(var i=0;i<data.list.length;i++){
					str = str + '<option value="'+data.list[i].menuId+'">'+data.list[i].menuName+'</option>';
				}
